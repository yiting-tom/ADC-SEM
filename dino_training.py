"""
DINOv2-style SSL training for TinyViT with multi-channel input.

Implements a student/teacher momentum model with multi-crop augmentations
and DINOLoss from lightly. Augmentations are geometric-only to support
>3-channel data.
"""
...
class MultiChannelDinoCollate:
    """Create multi-crop views for DINO-style training.

    - Two global crops at `global_size`
    - `num_local_crops` local crops at `local_size`
    """
    ...
    def __call__(self, batch) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        # Stack per-crop into batch tensors: [B, C, H, W] for each crop
        # The DataLoader collate_fn expects a single object; we return list of crops (each stacked over batch)
        batch_size = len(imgs)
...
class DinoTinyViT(nn.Module):
    """Student/Teacher TinyViT with DINO projection heads."""
    ...
    def __init__(self, backbone: TinyViTBackbone, proj_dim: int, hidden_dim: Optional[int] = None) -> None:
        ...
        # Teacher model: initialize from student via state_dict to avoid deepcopy issues
...
class DINOTrainer:
    def __init__(self, cfg: DINOTrainConfig) -> None:
        self.cfg = cfg
    ...
    def _build_dataloader(self) -> DataLoader:
        ...
        collate = MultiChannelDinoCollate(num_channels=self.cfg.num_channels, num_local_crops=self.cfg.local_crops)
        return DataLoader(
            ds,
    ...
    def _build_model(self) -> DinoTinyViT:
        ...
        model = DinoTinyViT(backbone=backbone, proj_dim=self.cfg.proj_dim)
        return model
    ...
    def train(self) -> Tuple[nn.Module, dict]:
        ...
        print("Starting DINOv2-style training")
        metrics = {"epoch_loss": []}
        for epoch in range(self.cfg.epochs):
            model.train()
            total_loss = 0.0
            # Cosine EMA momentum schedule from base -> 1.0
            ...
            for crops, _ in dataloader:
                ...
                # Teacher only on global crops (first two)
